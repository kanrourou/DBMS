...
int QE_TEST_1(vector<RID> &rids) {
	...
	{
		cout << "** TEST_RM_PRIVATE_1 failed :-( 1 **" << endl << endl;
		free(data);
		return -1;		
	}
	...
	{
		cout << "** TEST_RM_PRIVATE_1 failed :-( 2 **" << endl << endl;
		free(data);
		return -1;	
	}
	...
	if (createIndexforLeftB() != success)
	 {
		cout << "** TEST_RM_PRIVATE_1 failed :-( 3 **" << endl << endl;
		free(data);
		return -1;	
	}
	...
	for(int i = 0; i < tupleCount; i++)
	{
		if (i % 2 == 0)
		{
			...
			if (rc != success) 
			{
				cout << "** TEST_RM_PRIVATE_1 failed :-( 4 **" << endl << endl;
				free(data);
				return -1;
        		}
		}
	...
	while (leftIn->getNextTuple(data) != QE_EOF)
	{
		...
		if (val % 2 == 0)
		{
			cout << "**** Test Case 1 Failed :-( 5 ****" << endl;
			delete leftIn;			
			free(data);
			return -1;
		}		
